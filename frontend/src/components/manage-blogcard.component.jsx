...
const BlogStats = ({ stats }) => {
    ...
    return (
        <div className="flex max-lg:mb-6 max-lg:pb-6 gap-2  border-grey max-lg:border-b">
            {
                Object.keys(stats).map((key) => {
                    if (key.includes("parent")) return "";

                    visibleIndex++;

                    return (
                        <div key={key} className={"flex flex-col items-center w-full h-full justify-center p-4 px-6 " + (visibleIndex != 0 ? " border-grey border-l " : "")}>
    ...
    return (
        <>
            <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
                ...
                <img src={banner} className="max-md:hidden lg:hidden xl:block w-28 h-28 float-none bg-grey object-cover " />
...
const deleteBlog = (blog, access_token, target) => {
    ...
    axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/delete-blog", { blog_id }, {
        ...
        .then(({ data }) => {
            ...
            setStateFunc(preVal => {
                let { deleteDocCount = 0, totalDocs, results } = preVal;
